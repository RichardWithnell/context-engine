...
int route_cb(const struct nlmsghdr* nlh, struct update_obj** update)
{
    ...
    case RTM_NEWROUTE:
        print_verb("New Route - %s\n", ip_to_str(htonl(route->gateway)));
        u->action = ADD_RT;
        break;
    case RTM_DELROUTE:
        print_verb("Delete route - %s\n", ip_to_str(htonl(route->gateway)));
        u->action = DEL_RT;
        break;
    }


    if (!(u->update = malloc(sizeof(struct mnl_route)))) {
        print_error("ENOMOM\n");
        errno = ENOMEM;
        free(route);
        return errno;
    ...
    memcpy(u->update, route, sizeof(struct mnl_route));
    u->type = UPDATE_ROUTE;
    *update = u;
